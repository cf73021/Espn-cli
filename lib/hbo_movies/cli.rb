class HboMovies::CLI
  ...
  def call
    HboMovies::Scraper.new.make_restaurants
  ...
  def start
    ...
    print_restaurant(input)

    puts ""
    puts "What restaurant would you like more information on?"
    input = gets.strip

    restaurant = HboMovies::Movie.find(input.to_i)
  ...
  def print_restaurants(from_number)
    ...
    puts ""
    HboMovies::Movie.all[from_number-1, 10].each.with_index(from_number) do |restaurant, index|
